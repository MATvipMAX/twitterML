...
def create_atte_model(num_words, EMBEDDING_DIM, MAX_POST_LENGTH, embedding_matrix, MAX_POSTS):
...
def getModel():
    ...
    DIR = 'C:/Users/mateu/OneDrive/Pulpit/data/'
    ...
    for data_index in split_data.split(depress, 4):
        ...
        for train_index, test_index in skf.split(np.asarray(data), labels[:, 1]):
            ...
            print(class_weight)
            model = create_atte_model(num_words=num_words, EMBEDDING_DIM=EMBEDDING_DIM, MAX_POST_LENGTH=MAX_POST_LENGTH, embedding_matrix=embedding_matrix, MAX_POSTS = MAX_POSTS)

            model.fit(X_train, np.asarray(y_train, 'int32'), validation_data=(x_val, np.asarray(y_val, 'int32')),
                    shuffle=False,
                    epochs=200, batch_size=32, verbose=0, class_weight=class_weight,
