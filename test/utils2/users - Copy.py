...
import numpy as np
import time
from utils2.utils import clean_tweet
...
def load_users(_dir, post_size=100, max_posts=2000, nb_depress=500, return_df=False):
    ...
    print('Selecting users with at least {} posts'.format(post_size))
    ...
    # remove numbers
    control['clean'] = control['clean'].apply(clean_tweet)
    ...
    # remove numbers
    depress['clean'] = depress['clean'].apply(clean_tweet)
    print('-----------{}-----------'.format(time.time() - start))
